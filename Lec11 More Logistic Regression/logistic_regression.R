...
p0 <- ggplot(data=profiles, aes(x=1, y=is_female)) + geom_point()
p0
...
group_by(profiles, has.wine) %>% summarise(prop.female=mean(is_female))


# -b) Regression, coefficients, and fitted values: we now include the has.wine
# predictor variable
model1 <- glm(is_female ~ has.wine, data=profiles, family=binomial)
summary(model1)
...
# -c) Plot.
p1 <- ggplot(data=profiles, aes(x=has.wine, y=is_female)) + geom_point()
p1
...
# variables:
p1 <-
  ggplot(data=profiles, aes(x=jitter(as.numeric(has.wine)), y=jitter(is_female))) +
...
group_by(profiles, is_female) %>% summarise(mean=mean(height), SE=sd(height)/n())
...
p2 <- ggplot(data=profiles, aes(x=jitter(height), y=jitter(is_female))) +
...
# -b) Regression, coefficients, and fitted values
model3 <- glm(is_female ~ username.len, data=profiles, family=binomial)
summary(model3)
...
# -c) Plot:
p3 <- ggplot(data=profiles, aes(x=jitter(username.len), y=jitter(is_female))) +
